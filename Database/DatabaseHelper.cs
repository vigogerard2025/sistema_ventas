    ...
    public class DatabaseHelper
    {
        ...
        public static void InitializeDatabase()
        {
            using (var conn = GetConnection())
            {
                ...
                // ============================================================
                // PASO 2: Migrar columnas nuevas si no existen (SIN borrar datos)
                // ============================================================
                string sqlMigraciones = @"
                DO $$
                BEGIN
                    -- Agregar columna 'presentacion' a productos si no existe
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_name='productos' AND column_name='presentacion'
                    ) THEN
                    ...
                    -- Agregar columnas SUNAT a comprobantes si no existen
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.columns
                        WHERE table_name='comprobantes' AND column_name='sunat_estado'
                    ) THEN
